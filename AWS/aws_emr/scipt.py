...
def transform_data(database: str, table_source: str, table_target) -> None:
    ...
    df = spark.read.table(f"'{dabase}'.'{table_source}'")
    ...
    df = df.withColumn("delay_day", when
                        (col("closed_dt") > col("target_dt"),
                        (unix_timestamp("closed_dt")-unix_timestamp("target_dt"))/86400,
                        ).otherwise(0),
                      )
    ...
    #Salvar em uma tabela temporária pra depois conseguir salvar no Glue DataCatalog
    ...
    result_df.write.mode("overwrite").format("parquet").saveAsTable(f"'{dabase}'.'{table_target}'")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Transformar dados de solicitação de serviço de Boston 311"
    )
